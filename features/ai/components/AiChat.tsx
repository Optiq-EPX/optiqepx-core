...
export function AiChat({ classLevel, username, userAvatar }: { classLevel: string, username: string, userAvatar?: string | null }) {
  ...
  return (
    <div className="flex flex-col flex-1 w-full relative">
      ...
      <div ref={scrollRef} className="flex-1 w-full pb-32">
              ...
              >
                ...
                <div className={cn(
                  ...
                  <motion.div 
                    ...
                    transition={{ type: "spring", damping: 30, stiffness: 200 }}
                    className={cn(
                      "p-5 sm:p-6 rounded-[1.75rem] font-outfit text-sm sm:text-base leading-relaxed transition-all stable-bubble min-h-[44px] min-w-0 overflow-hidden",
                      msg.role === 'user' 
                        ? "bg-gradient-to-br from-violet-600 to-indigo-600 text-white rounded-tr-sm shadow-xl shadow-violet-500/10" 
                        : "bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-tl-sm text-foreground/90 shadow-sm backdrop-blur-md w-full"
                    )}
                  >
                    {msg.role === 'assistant' ? (
                      <MarkdownRenderer content={msg.content} isGenerating={isLoading && idx === messages.length - 1} />
                    ) : (
      ...
      <div className="mt-auto sticky bottom-4 z-50 px-2 sm:px-4">
          <div className="relative w-full max-w-4xl mx-auto rounded-2xl shadow-[0_8px_40px_-10px_rgba(0,0,0,0.8)] bg-black/40 backdrop-blur-xl border border-white/10">
            <div className="relative flex items-end gap-2 sm:gap-3 w-full p-2">
              
              <div className="relative shrink-0" ref={modeRef}>
                  <button
                  onClick={() => setModeOpen(!modeOpen)}
                  className="h-[52px] w-[52px] sm:w-auto flex items-center justify-center sm:justify-start gap-2 px-0 sm:px-5 rounded-2xl bg-white/[0.08] backdrop-blur-sm border border-white/[0.12] hover:border-violet-500/30 text-violet-400 font-space-grotesk font-bold text-[11px] uppercase tracking-wider cursor-pointer whitespace-nowrap transition-all shadow-[0_2px_20px_-4px_rgba(139,92,246,0.15)]"
                >
                  {(() => { const active = modes.find(m => m.id === mode); return active ? <active.icon className="w-4 h-4 sm:w-4 sm:h-4 shrink-0" /> : null; })()}
                  <span className="hidden sm:inline">{modes.find(m => m.id === mode)?.label}</span>
                  <ChevronUp className={cn(
                    "transition-transform opacity-40 hidden sm:block w-3.5 h-3.5 ml-0.5",
                    modeOpen ? "rotate-180" : ""
                  )} />
                </button>

                {modeOpen && (
                  <div className="absolute bottom-full left-0 mb-3 w-60 bg-zinc-950 border border-white/[0.08] rounded-2xl shadow-[0_8px_40px_-8px_rgba(0,0,0,0.7)] overflow-hidden z-50">
              ...
              <div className="flex-1 relative rounded-2xl bg-white/[0.03] border border-white/[0.08] shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)] focus-within:border-violet-500/30 focus-within:shadow-[inset_0_2px_4px_rgba(0,0,0,0.3),0_0_0_1px_rgba(139,92,246,0.1)] transition-all flex overflow-hidden">
                <Textarea 
                  value={input}
                  ...
                  className="w-full min-h-[52px] max-h-[200px] bg-transparent border-0 focus-visible:ring-0 rounded-2xl resize-none px-5 py-4 text-[15px] font-outfit font-medium text-zinc-200 placeholder:text-zinc-600"
                />
              </div>
              ...
              <div className="shrink-0 flex items-center gap-2">
                {editingIndex !== null && !isLoading && (
                  <Button 
                    ...
                    className="h-[52px] w-[52px] rounded-2xl bg-white/[0.08] backdrop-blur-sm border border-white/[0.12] hover:bg-white/[0.1] text-zinc-400 p-0 transition-all cursor-pointer flex items-center justify-center group/cancel"
                    title="Cancel editing"
                  >
                    <X className="w-5 h-5 group-hover/cancel:scale-110 transition-transform" />
                  </Button>
                )}
                {isLoading ? (
                  <Button 
                    variant="ghost"
                    onClick={handleStop}
                    className="h-[52px] w-[52px] rounded-2xl bg-white/[0.08] backdrop-blur-sm border border-white/[0.12] hover:border-red-500/30 hover:bg-red-500/10 text-red-500/70 shadow-xl p-0 transition-all cursor-pointer flex items-center justify-center group/stop"
                  >
                    <Square className="w-4 h-4 fill-current text-red-500/70 group-hover/stop:scale-110 group-hover/stop:text-red-500 transition-all" />
                  </Button>
                ) : (
                  <Button 
                    variant="ghost"
                    disabled={!input.trim()} 
                    onClick={() => handleSend()}
                    className="h-[52px] w-[52px] rounded-2xl bg-white/[0.08] backdrop-blur-sm border border-white/[0.12] hover:border-violet-500/30 hover:bg-violet-500/10 text-violet-400 shadow-[0_2px_20px_-4px_rgba(139,92,246,0.15)] p-0 transition-all cursor-pointer flex items-center justify-center disabled:opacity-100 disabled:cursor-default"
                  >
                    <Send className={cn("w-[18px] h-[18px] transition-opacity", !input.trim() && "opacity-20")} />
                  </Button>
                )}
              </div>
            </div>
